...
DECLARE @INCRI2 VARCHAR(MAX) = CONVERT(VARCHAR(MAX),@CLAVE2,2)
 ...
 SELECT t1.usua_Id, t1.usua_EsAdmin , T2.pers_Nombres, t2.pers_Id FROM Gral.tblUsuarios T1 INNER JOIN Gral.tblPersonas T2 ON T1.usua_Empleado = T2.pers_Id WHERE 
...
AS
BEGIN
  SELECT pers_Id, pers_Nombres, pers_Apellidos, pers_DNI, pers_Puesto, 
CASE WHEN pers_Sexo = 'F' THEN 'Femenino' ELSE 'Masculino' END as pers_Sexo, 
T2.esci_Descripcion
FROM Gral.tblPersonas T1 INNER JOIN Gral.tblEstadoCiviles T2 ON T1.pers_EstadoCivil = T2.esci_ID
